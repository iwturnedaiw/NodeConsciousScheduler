...
//            System.out.println("Job Id: " + ev.getJob().getJobId() + ", Submission time " + ev.getJob().getSubmitTime());
    ...
    private void outputWaitingAndNewArrivalJobAndStartJobAndFinishedJob(String fileNameWaiting, String fileNameArrival, String fileNameStart, String fileNameCumulativeStart, String fileNameFinished, String fileNameCumulativeFinished, String fileNameWaitingRscs, String fileNameExecutingRscs, int MODE) {
            ...
            printTimeSeriesToFile(fileNameWaitingRscs, resultCumulativeWaitingRscs, resultCumulativeStartedRscs);
            printTimeSeriesToFile(fileNameExecutingRscs, resultCumulativeStartedRscs, resultCumulativeFinishedRscs);
    }
    ...
    private void printByUser(ArrayList<UserResult> resultEachUser) {
        ...
        try {
            ...
            pw.print("userId\tnumJob\trunningTime\trunningTime(ave.)\trunningTime(max)\twaitTime\twaitTime(ave.)\twaitTime(max)\tnumNode\tnumNode(ave.)\tnumCore\tnumCore(ave.)\t"
            ...
            for (int i = 0; i < resultEachUser.size() ; ++i) {
                ...
                pw.print(userId + "\t" + numJob + "\t" + runningTime + "\t" + averagedRunningTime + "\t" + maxRunningTime + "\t" + waitTime + "\t" + averagedWaitTime + "\t" + maxWaitTime + "\t" +
    ...
    private void calculateUserResult(Map<Integer, ArrayList<Job>> jobIdEachUser, ArrayList<UserResult> resultEach) {
        for (Integer userId: jobIdEachUser.keySet()) {
            ...
            resultEach.add(new UserResult(userId, numJob, accumulatedNumNode, averagedNumNode, accumulatedNumCore, averagedNumCore, 
                                              ...
                                              accumulatedWaitTime, averagedWaitTime, largeJobRatio, slowdownHistgramEachUser, slowdownOCHistgramEachUser, maxRunningTime, maxWaitTime, maxCpuTime));
        }        
    }
    ...
    private void printByGroup(ArrayList<GroupResult> resultEachGroup) {
        ...
        try {
            ...
            pw.print("groupId\tnumJob\trunningTime\trunningTime(ave.)\trunningTime(max)\twaitTime\twaitTime(ave.)\twaitTime(max)\tnumNode\tnumNode(ave.)\tnumCore\tnumCore(ave.)\t"
                     + "CpuTime\tCpuTime(ave.)\tCpuTime(max)\tMemoryFootprint\tMemoryFootprint(ave)\tlargeJobRatio\t");
            ...
            for (int i = 0; i < resultEachGroup.size() ; ++i) {
                ...
                pw.print(groupId + "\t" + numJob + "\t" + runningTime + "\t" + averagedRunningTime + "\t" + maxRunningTime + "\t" + waitTime + "\t" + averagedWaitTime + "\t" + maxWaitTime + "\t" +
                           numNode + "\t" + averagedNumNode + "\t" + numCore + "\t" + averagedNumCore + "\t" + cpuTime + "\t" + averagedCpuTime + "\t" + maxCpuTime + "\t" + 
                           memoryFootprint + "\t" + averagedMemoryFootprint + "\t" + largeJobRatio + "\t");
    ...
    private void printTimeSeriesToFile(String fileNameRequireRscs, ArrayList<Long> resultRscs1, ArrayList<Long> resultRscs2) {
        ArrayList<Double> resultReduced = new ArrayList<Double>();
        long systemRscs = NodeConsciousScheduler.numNodes*NodeConsciousScheduler.numCores;
